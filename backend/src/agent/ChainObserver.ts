...
class ChainObserverSystem {
  ...
  private readonly boundOnBlockProduced: (payload: any) => Promise<void>;
  private readonly boundOnTransactionAdded: (tx: any) => Promise<void>;
  private readonly boundOnConsensusFailed: (data: any) => Promise<void>;
  ...
  private async onBlockProduced(payload: any): Promise<void> {
  ...
  private async onTransactionAdded(tx: any): Promise<void> {
    ...
    if (value > BigInt(10000)) {
      await agentMemory.recordObservation(
        `Large transaction detected: ${value.toString()} HERMES`,
