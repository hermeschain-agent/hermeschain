...
export class BrowserAutomation {
  ...
  async browseUrl(url: string, extractText: boolean = true): Promise<BrowserResult> {
    const startTime = Date.now();
    
    try {
      // Use curl to fetch the page
      const response = execSync(
        `curl -sL -A "${this.userAgent}" -w "\\n%{http_code}\\n%{time_total}" --max-time 30 "${url}"`,
  ...
  async screenshotUrl(
    ...
    try {
      ...
      execSync(`node -e "${puppeteerScript.replace(/\n/g, ' ')}"`, {
      ...
      try {
        execSync(`wkhtmltoimage --quality 80 "${url}" "${outputPath}"`, {
          timeout: 60000
        });
  ...
  async checkDeployment(url: string, expectedText?: string): Promise<BrowserResult> {
    const startTime = Date.now();
  ...
  async searchWeb(query: string, numResults: number = 5): Promise<BrowserResult> {
    try {
      const encodedQuery = encodeURIComponent(query);
      const searchUrl = `https://html.duckduckgo.com/html/?q=${encodedQuery}`;

      const response = execSync(
        `curl -sL -A "${this.userAgent}" --max-time 15 "${searchUrl}"`,
      ...
      if (results.length === 0) {
        ...
        while ((match = linkRegex.exec(response)) !== null) urls.push(match[1]);
        while ((match = titleRegex.exec(response)) !== null) titles.push(match[1]);
  ...
  async extractLinks(url: string): Promise<BrowserResult> {
    try {
      ...
      const html = execSync(
        `curl -sL -A "${this.userAgent}" --max-time 30 "${url}"`,
        { encoding: 'utf-8', timeout: 35000, maxBuffer: 10 * 1024 * 1024 }
      );
  ...
  async executeTool(toolName: string, args: any): Promise<BrowserResult> {
