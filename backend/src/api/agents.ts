...
import { hermesChatCompletion } from './open';
...
const reviewAgent = async (agent: Partial<Agent>): Promise<{ approved: boolean; reason: string }> => {
  const systemPrompt = `You are reviewing a user-created AI agent for Hermeschain. 
...
Should this agent be deployed on Hermeschain?`;

  try {
    const response = await hermesChatCompletion(systemPrompt, userMessage);
...
agentsRouter.post('/create', async (req, res) => {
  try {
    ...
    res.json({
      success: true,
      agentId,
      message: `${agent.name} has been deployed to Hermeschain!`,
...
agentsRouter.post('/:id/chat', async (req, res) => {
  try {
    ...
    const systemPrompt = `You are ${agent.name}, a ${agent.role} on Hermeschain.
...
You are an AI agent deployed on Hermeschain, a blockchain run entirely by AI. Stay in character and respond based on your defined personality and philosophy. Be helpful but maintain your unique perspective.

Keep responses concise (under 200 words) and relevant to blockchain/crypto/AI topics when possible.`;

    const response = await hermesChatCompletion(systemPrompt, message);
