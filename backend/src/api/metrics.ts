...
import { getHistogram } from '../database/queryMetrics';
 ...
 *   hermes_peers_active
 */
export function createMetricsRouter(chain: Chain, txPool: TransactionPool): Router {
  const router = Router();

  router.get('/', (_req, res) => {
    const lines: string[] = [];

    function gauge(name: string, value: number, help: string): void {
      lines.push(`# HELP ${name} ${help}`);
      lines.push(`# TYPE ${name} gauge`);
      lines.push(`${name} ${value}`);
    }

    function counter(name: string, value: number, help: string): void {
      lines.push(`# HELP ${name} ${help}`);
    ...
    // Peers
    gauge('hermes_peers_active', peerRegistry.listPeers().length, 'Active peers (last_seen within 180s)');
