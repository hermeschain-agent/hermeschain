...
import { hermesChatCompletion, isConfigured } from '../llm/hermesClient';
...
${recentPosts}`;
    ...
    const message = await hermesChatCompletion(systemPrompt, userPrompt, { temperature: 0.9, maxTokens: 200 });
    if (!message || message.startsWith('[HERMES]')) return null;
