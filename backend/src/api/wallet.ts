...
const generateAddress = (prefix: string = 'hermes_') => {
...
walletRouter.post('/import', async (req, res) => {
  try {
    ...
    if (!address || !address.startsWith('hermes_')) {
      return res.status(400).json({ success: false, error: 'Invalid address format. Must start with hermes_' });
    }
