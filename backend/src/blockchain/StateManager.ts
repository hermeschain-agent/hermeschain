...
const INITIAL_SUPPLY = BigInt('1000000000000000000000000'); // 1 million HERMES (with 18 decimals)
const GENESIS_ADDRESS = 'OPENGenesis1111111111111111111111111111111';
const FAUCET_ADDRESS = 'HERMESCHAIN_FAUCET';
...
export class StateManager {
  ...
  async initialize(): Promise<void> {
    ...
    try {
      ...
      console.log(`[STATE] Faucet balance: ${this.formatBalance(this.getBalance(FAUCET_PUBLIC_ADDRESS))} HERMES`);
   ...
   */
  async revertBlock(
    block: { transactions: Transaction[]; header: { producer: string; height: number } },
    blockReward: bigint = 10n * 10n ** 18n,
    ...
    if (producerAcct) {
      if (producerAcct.balance >= blockReward) {
        ...
        console.warn(
          `[STATE] Producer ${producer.slice(0, 12)}... balance ${producerAcct.balance} < reward ${blockReward} on revert`,
        );
      }
    ...
    for (let i = block.transactions.length - 1; i >= 0; i -= 1) {
      ...
      fromAccount.balance += totalCost;
      if (fromAccount.nonce > 0) fromAccount.nonce -= 1;
      ...
      if (toAccount.balance >= tx.value) {
        ...
        // it defensively.
        console.warn(
          `[STATE] Recipient ${tx.to.slice(0, 12)}... balance ${toAccount.balance} < tx.value ${tx.value} on revert — clamping`,
        );
  ...
  getAccountsSummary(): { address: string; balance: string; nonce: number }[] {
    return Array.from(this.accounts.values())
      ...
      .sort((a, b) => {
        const balA = BigInt(a.balance.replace(' HERMES', '').replace(/,/g, ''));
        const balB = BigInt(b.balance.replace(' HERMES', '').replace(/,/g, ''));
  ...
  formatBalance(balance: bigint): string {
    ...
    return `${wholePart.toLocaleString()} HERMES`;
  }
