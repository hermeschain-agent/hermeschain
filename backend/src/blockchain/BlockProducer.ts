...
import { interpreter, parseVmProgram } from '../vm/Interpreter';
import type { Log } from './TransactionReceipt';
    ...
    try {
      ...
      for (const tx of pendingTxs) {
        ...
        if (applied) {
          ...
          const program = parseVmProgram(tx.data);
          let txGasUsed = tx.gasLimit;
          let txLogs: Log[] = [];
          let txStatus = TransactionStatus.SUCCESS;

          if (program) {
            const result = interpreter.execute(program, tx.gasLimit, {
            ...
            if (result.status === 'revert') {
              txStatus = TransactionStatus.OUT_OF_GAS;
