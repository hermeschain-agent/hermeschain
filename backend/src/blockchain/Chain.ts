...
export class Chain {
  ...
  private transactionPool: TransactionPool | null = null;
  ...
  async addBlock(block: Block): Promise<boolean> {
    ...
    if (
      ...
      const fork = forkManager.addBlock(block, {
        valid: true,
        confidence: 1,
      ...
      if (fork.orphaned.length > 0) {
        ...
        console.warn(
          `[CHAIN] Block ${block.header.height} orphaned (parent ${block.header.parentHash.slice(0, 12)}... not on chain)`,
        );
        return false;
      }
  ...
  async handleReorg(newBlocks: Block[], commonAncestorHeight: number): Promise<{
    ...
    for (let i = result.orphaned.length - 1; i >= 0; i -= 1) {
      try {
        await stateManager.revertBlock(result.orphaned[i]);
      } catch (err) {
        console.error(
          `[CHAIN] revertBlock failed for orphan ${result.orphaned[i].header.height}:`,
          err,
        );
      }
    }
