...
const BUILT_IN_SKILLS = [
    ...
    },
    {
        ...
        description: 'Interact with Hermeschain blockchain',
        version: '1.0.0',
        enabled: true,
        systemPromptAddition: `You have access to Hermeschain blockchain operations:
...
class SkillManager {
    ...
    parseSkillMarkdown(mdPath, dirName) {
        try {
    ...
    // Execute a trigger
    async executeTrigger(skill, trigger, data) {
        console.log(`[SKILLS] Trigger fired for ${skill.name}: ${trigger.value}`);
        EventBus_1.eventBus.emit('skill_trigger', {
            skill: skill.id,
    ...
    addSkill(skill) {
        ...
        EventBus_1.eventBus.emit('skill_added', { skill: skill.id });
    }
    // Remove a skill
    removeSkill(id) {
        if (this.skills.has(id)) {
            ...
            EventBus_1.eventBus.emit('skill_removed', { skill: id });
            return true;
        }
        return false;
    }
