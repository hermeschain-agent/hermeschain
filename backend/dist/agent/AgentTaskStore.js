...
exports.agentTaskStore = exports.AgentTaskStore = void 0;
...
class AgentTaskStore {
    ...
    async initialize() {
        if (this.initialized)
            return;
        try {
            await db_1.db.exec(CREATE_TASK_TABLES);
    ...
    async startRun(task, mode, contextSummary) {
        await this.initialize();
        const run = {
    ...
    async finishRun(runId, status, verificationStatus, details = {}) {
        ...
        if (!updated)
            return null;
        const sourceStatus = status === 'succeeded'
            ? 'succeeded'
            : status === 'discarded'
                ? 'discarded'
                : status === 'blocked'
    ...
    async markSourceTaskInProgress(sourceTaskId) {
    ...
    getRecentSuccessfulRuns(limit = 10) {
        return this.getRecentRuns(200)
            .filter((run) => run.status === 'succeeded')
            .slice(0, limit);
    }
    getRecentFailedRuns(limit = 10) {
        return this.getRecentRuns(200)
            .filter((run) => run.status === 'failed' || run.status === 'blocked')
            .slice(0, limit);
    }
    getCurrentRun() {
        return (this.taskRuns.find((run) => run.status === 'selected' ||
            run.status === 'analyzing' ||
            run.status === 'executing' ||
            run.status === 'verifying') || null);
    }
    async requeueSourceTask(id) {
        return this.updateSourceTaskStatus(id, 'queued', {
            lastError: null,
            blockedReason: null,
        });
    }
    async discardSourceTask(id, reason) {
        return this.updateSourceTaskStatus(id, 'discarded', {
            blockedReason: reason,
        });
    }
    getBacklogProgress(totalTasks) {
        const completed = Array.from(this.sourceTasks.values()).filter((task) => task.source === 'backlog' && task.status === 'succeeded').length;
        return {
            completed,
            total: totalTasks,
...
exports.agentTaskStore = new AgentTaskStore();
//# sourceMappingURL=AgentTaskStore.js.map