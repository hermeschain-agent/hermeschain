...
async function main() {
    ...
    app.get('/api/contract/:addr/disasm', async (req, res) => {
        try {
            const r = await db_1.db.query(`SELECT bytecode, code_hash, deployed_at_block, deployed_by
           FROM contract_code WHERE address = $1`, [req.params.addr]);
            if (r.rows.length === 0)
                return res.status(404).json({ error: 'no code at address' });
            const row = r.rows[0];
            let ops = null;
            try {
                ops = JSON.parse(row.bytecode);
            }
            catch { /* leave null */ }
            res.json({
    ...
    app.get('/api/contract/:addr/storage', async (req, res) => {
        try {
            ...
            const r = await db_1.db.query(`SELECT storage_key, storage_value, updated_at_block
           FROM contract_storage
          WHERE contract_address = $1 AND storage_key LIKE $2
          ORDER BY storage_key
          LIMIT $3`, [req.params.addr, prefix + '%', limit]);
            res.json({
                address: req.params.addr,
    ...
    app.get('/api/chain/export', async (req, res) => {
        ...
        const to = Math.min(from + 1000000, Number(req.query.to ?? from + 1000));
    ...
    const { createJsonRpcRouter } = await Promise.resolve().then(() => __importStar(require('./jsonrpc')));
