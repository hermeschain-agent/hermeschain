...
async function initializeAuthTables() {
    ...
    `);
        ...
        const result = await db_1.db.query('SELECT * FROM api_keys WHERE active = true');
...
authRouter.post('/keys', async (req, res) => {
    try {
        ...
        const id = `key_${Date.now().toString(36)}${crypto_1.default.randomBytes(4).toString('hex')}`;
...
authRouter.delete('/keys/:id', async (req, res) => {
    try {
        ...
        await db_1.db.query('UPDATE api_keys SET active = false WHERE id = $1', [id]);
        apiKeys.delete(id);
...
function checkRateLimit(identifier, limit = defaultRateLimit.maxRequests) {
    const now = Date.now();
...
function getRateLimitRemaining(identifier, limit = defaultRateLimit.maxRequests) {
    const bucket = rateLimitBuckets.get(identifier);
...
function apiKeyAuth(requiredPermission) {
    return async (req, res, next) => {
        ...
        db_1.db.query('UPDATE api_keys SET last_used = $1, usage_count = usage_count + 1 WHERE id = $2', [foundKey.lastUsed, foundKey.id]).catch(() => { });
...
function ipRateLimit(maxRequests = 30) {
    return (req, res, next) => {
...
authRouter.post('/sessions', async (req, res) => {
    try {
        ...
        const id = `sess_${Date.now().toString(36)}${crypto_1.default.randomBytes(4).toString('hex')}`;
...
authRouter.get('/sessions/validate', async (req, res) => {
    ...
    if (!foundSession) {
        // Check database
        try {
            const result = await db_1.db.query('SELECT * FROM user_sessions WHERE token_hash = $1 AND expires_at > NOW()', [tokenHash]);
    ...
    db_1.db.query('UPDATE user_sessions SET last_activity = NOW() WHERE id = $1', [foundSession.id]).catch(() => { });
    res.json({
        valid: true,
        session: {
            id: foundSession.id,
...
authRouter.delete('/sessions/:id', async (req, res) => {
    try {
        const { id } = req.params;
        sessions.delete(id);
        await db_1.db.query('DELETE FROM user_sessions WHERE id = $1', [id]);
